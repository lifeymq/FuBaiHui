...
import com.example.lenovo.fubaihui.bean.County;
import com.example.lenovo.fubaihui.bean.GetJsonDataUtil;
...
import com.example.lenovo.fubaihui.frame.RecyclerAdapter_Integral_show;
import com.example.lenovo.fubaihui.frame.RecyclerAdapter_pop_show;
...
public class IntegralActivity extends AppCompatActivity implements ICommonView,RecyclerAdapter_Integral_show.Pull,RecyclerAdapter_pop_show.Pult {
    ...
    private void showPickerView() {// 弹出选择器（省市区三级联动）
        OptionsPickerView pvOptions = new OptionsPickerBuilder(this, new OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                Log.e("------",id1.get(options1)+"|"+id2.get(options1).get(options2)+id3.get(options1).get(options2).get(options3));
                //返回的分别是三个级别的选中位置
                ...
                mPresenter.getData(ApiConfig.GET_PERSON_RANKING_RESULT_INTEGRAL_SHOW,id1.get(options1),id2.get(options1).get(options2),id3.get(options1).get(options2).get(options3));
    ...
    public void onSuccess(int whichApi, Object successResult) {
        switch (whichApi){
            case ApiConfig.GET_PERSON_RANKING_RESULT_INTEGRAL:
                ...
                ArrayList<Integral.DataBean.DpflBean> dpflBeans= (ArrayList<Integral.DataBean.DpflBean>) info.getData().getDpfl();
                ...
                for (int j = 0; j < ssx.size(); j++) {
                    ...
                    ArrayList<ArrayList<String>> Province_AreaList_id = new ArrayList<>();//该省的所有地区id（第三级）
                    ...
                    for(int i=0;i<ssxBean.getShi().size();i++){
                        ...
                        if (shiBean.getXian() != null && shiBean.getXian().size() > 0){
                            for (int k = 0; k < shiBean.getXian().size(); k++) {
                                Integral.DataBean.SsxBean.ShiBean.XianBean xianBean = shiBean.getXian().get(k);
                                City_AreaList.add(ssx.get(j).getShi().get(i).getXian().get(k).getRegion_name());
                                City_AreaList_id.add(ssx.get(j).getShi().get(i).getXian().get(k).getRegion_id());
                            }
                        }else {
                            City_AreaList.add("");
    ...
    private void showpopupwindow(){
        ...
        RecyclerAdapter_pop_show recyclerAdapter_pop_show=new RecyclerAdapter_pop_show(poplist,this);
    ...
    private void show() {
        ...
        RecyclerAdapter_Integral_show recyclerAdapter_integral_show=new RecyclerAdapter_Integral_show(integralshowlist,this);
        LinearLayoutManager layoutManager=new LinearLayoutManager(this);
    ...
    public void put(int position) {
        ...
        CommonPresenter mPresenter=new CommonPresenter();
        mPresenter.bind(this, new TestModel());
        mPresenter.getData(ApiConfig.GET_PERSON_RANKING_RESULT_INTEGRAL_SHOW_POP,popidlist.get(position));
    }
}

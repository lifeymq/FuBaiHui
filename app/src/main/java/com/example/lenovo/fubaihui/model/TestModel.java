   ...
   public void getData(final int whichApi, final ICommonView presenterCallBack, Object[] params) {
      ...
      switch (whichApi) {
         ...
         case ApiConfig.GET_CODE:
            manager.method(manager.getNetService(Config.BASEURL).getCode((String) params[0]),
                whichApi, presenterCallBack);
            break;
         case ApiConfig.GET_SIGN:
            manager.method(manager.getNetService(Config.BASEURL).getSign((String) params[0],
                (String) params[1]), whichApi, presenterCallBack);
            break;
         case ApiConfig.GET_MODIFY:
            manager.method(manager.getNetService(Config.BASEURL).getModify((String) params[0],
                (String) params[1],
                (String) params[2], (String) params[3]), whichApi, presenterCallBack);
            break;
         case ApiConfig.GET_INVITATION:
            manager.method(manager.getNetService(Config.BASEURL).getInvitationcode((String)
                params[0]), whichApi, presenterCallBack);
            break;
         //            case ApiConfig.GET_PERSON_RANKING_RESULT_TEST:
         //                manager.method(manager.getNetService(Config.BASEURL).getTopic(),
         // whichApi,presenterCallBack);
         //                break;
         case ApiConfig.GET_STORE_CLASSIFY:
            manager.method(manager.getNetService(Config.BASEURL1).getStoreClassify(), whichApi,
                presenterCallBack);
            break;
         case ApiConfig.GET_STORE_LIST:
            int cid = (int) params[0];
            manager.method(manager.getNetService(Config.BASEURL1).getStoreList(cid), whichApi,
                presenterCallBack);
            break;
         case ApiConfig.GET_STORE_PARTICULARS:
            int id = (int) params[0];
            manager.method(manager.getNetService(Config.BASEURL1).getStoreParticulars(id),
                whichApi, presenterCallBack);
            break;

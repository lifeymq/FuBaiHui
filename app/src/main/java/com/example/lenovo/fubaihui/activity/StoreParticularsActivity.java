      ...
      switch (whichApi) {
         case ApiConfig.GET_STORE_PARTICULARS:
            ...
            if (data.getImages().size() == 1) {
               Glide.with(this).load("http://newwasj.zhangtongdongli.com" + data.getImages().get
                   (0)).error(R.mipmap.ic_launcher).placeholder(R.mipmap.ic_launcher)
                   .diskCacheStrategy(DiskCacheStrategy.NONE).into(images1);
               //                    Glide.with(this).load("http://newwasj.zhangtongdongli.com" +
               // data.getImages().get(0)).fitCenter().into(images1);
            } else if (data.getImages().size() == 2) {
               Glide.with(this).load("http://newwasj.zhangtongdongli.com" + data.getImages().get
                   (0)).error(R.mipmap.ic_launcher).placeholder(R.mipmap.ic_launcher)
                   .diskCacheStrategy(DiskCacheStrategy.NONE).into(images1);
               //                    Glide.with(this).load("http://newwasj.zhangtongdongli.com" +
               // data.getImages().get(1)).error(R.mipmap.ic_launcher).placeholder(R.mipmap
               // .ic_launcher).diskCacheStrategy(DiskCacheStrategy.NONE).into(images2);
               ...
               //下载图片保存到本地
               Glide.with(this)
                   .load("http://newwasj.zhangtongdongli.com" + data.getImages().get(1))
                   .fitCenter().downloadOnly(new SimpleTarget<File>() {
                  @Override
                  public void onResourceReady(@NonNull File resource, @Nullable Transition<?
                      super File> transition) {
                     // 将保存的图片地址给SubsamplingScaleImageView,这里注意设置ImageViewState设置初始显示比例
                     images2.setImage(ImageSource.uri(Uri.fromFile(resource)), new ImageViewState
